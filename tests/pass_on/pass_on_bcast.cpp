...
{
  return CnC::COMPUTE_ON_ALL_OTHERS;
}
...
int main( int, char*[] )
{
    ...
    c.wait();
    assert( ( numProcs == 1 && c.m_items.size() == N - 1 ) || c.m_items.size() == ( numProcs - 1 ) * ( N - 1 ) || NULL == "Number of elements in the item collection is incorrect!");
